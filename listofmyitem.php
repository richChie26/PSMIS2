...
,ifnull((SELECT amount FROM `t_itemreceived` xy 
left join t_receivedheader zx on xy. `transcode` = zx.transcode
where `itemid` =  a.id and  `ResponsibilityCenter` = $Responsibility and convert(datereceived,date) <= convert('$datereceived',date)  
limit 1 ),0) cost
FROM `m_materials` a 
...
where a.isactive = 1 and d.id = $optcount order by a.description Asc 
"; 
//  echo $sql;
...
while($row = mysqli_fetch_array($qry)){
	echo '<tr class= "itemrownew" id="'.$row['id'].'|'.$row['stockno'].'|'.$row['item'].'|'.$row['description'].'|'.$row['units'] .'|'.$row['accounttitle'] .'|'.$row['bal']  .'|'.$row['reorderpoint'] .'|'.$row['cost'] .'" >
