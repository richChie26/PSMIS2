...
where a.isactive = 1 and `createdby`  = $uid  order by a.id desc ";
	$qry = mysqli_query($con,$sql);
$forprint = "";
while ($row = mysqli_fetch_array($qry)) {

if($row['status'] ==  'Released'){
$forprint = '<a href="#" class="btnprintris" id="ptr'. '|'. $row['risno'] .'"><span class="glyphicon glyphicon-print" style="color:#337ab7;"></span></a>';
}else{
  $forprint = "";
}
...
description, case when approvedqty = 0 then 
`qty` 
else 
approvedqty
end qty 
