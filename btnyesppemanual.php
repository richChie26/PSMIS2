...
, `amount`, `decription`, `cha`, `serial`,Fundcategory,a.itemid ,a.fund,dateaquire,
a.propertyno
...
where tag = 0  and a.createdby = $uid
...
FROM `t_transferouside` WHERE ifnull(substring(`creationdate`,1,4),substring(now(),1,4))  = substring(now(),1,4) and tag  = 0 ";
...
mysqli_query($con,$sqlinsertheader);
	$sqlheader ="insert into  `t_transferouside` (`origin`, `ptrcode`, `ptrdate`, `resposibilityid`, `isactive`, `createdby`, `creationdate`, `tcode`,tag)
values
 ('$txtorigin', '$PTRNumber', '$ptrdate', $Responsibility, 1, $uid, now(), '$code',0)";
...
(`tcode`, `propertyno`, `responsibilitycenter`, `itemid`, `amount`, `description`, `cha`, `serial`, `dateacquired`, `fundid`, `tag`, `isactive`, `createdby`, `creationdate`)
values
('$code', '$Property', $Responsibility, $itemid, $amount, '$decription', '$cha', '$serial', '$dateaquire', $fund, 0, 1, $uid, now())
";
...
$sqlinsertdetails = "insert into 
 `t_equipmentdeliverydetails`(`ResponsibilityCenter`, `deliverycode`, `itemid`, `propertyno`, `dateaquired`, `description`, `Serial`, `amount`, `isactive`, `createdby`, `creationdate`,chasisnumber)
values($Responsibility,'$code', $itemid, '$Property', '$dateaquire', '$decription', '$serial', $amount , 1, $uid , now(),'$cha')";
