...
include 'cn.php';
  ...
  if(!isset($_SESSION['uid']) || (trim($_SESSION['uid']) == '')) {
    ...
    $uid = $_SESSION["uid"] ;
     $typeofasset= $_GET['mnuasset'];

    $sqlxxx = "SELECT `typeofasset` FROM `m_accouttitle` 
    WHERE isactive = 1 and `accounttitle` = '$typeofasset'";

    $qryxxx = mysqli_query($con,$sqlxxx);

    $myr = mysqli_fetch_array($qryxxx);
    $mnuasset = $myr['typeofasset'];
    $yourppe = "";
    if($mnuasset == 2){
...
while($row = mysqli_fetch_array($qry)){
	echo '<tr class= "itemroweqxs" id="'.$row['id'].'|'.$row['accounttitle'].'|'.$row['item'].'|'.$row['yearoflife'].'" >
