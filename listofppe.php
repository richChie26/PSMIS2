...
$sqlppe = "select * from (SELECT a.id, d.accounttitle,item,yearoflife, 
concat(item, ', ',`description`,', ', `Serial`,', ', `chasisnumber`) description, `propertyno`
,ifnull((SELECT f.`propertyno` FROM `t_temppar` f  WHERE f.`propertyno` = a.propertyno),'N/A') NewTag
...
where a.isactive = 1 and `ResponsibilityCenter` = $Responsibility and ifnull(a.`Status`,'') in ('','Serviceable','For Repair','For Disposal')
and b.typeofasset = 3) ne where NewTag = 'N/A'
...
<div class= "row"> <div class="col-xs-12"><table class ="table table-striped display" id="example"
>
...
while($row = mysqli_fetch_array($qrypee)){
	echo '<tr class= "itemrowppe" id="'.$row['id'].'|'.$row['propertyno'].'|'.$row['item'].'|'.$row['description'].'|'.$row['yearoflife'] .'|'.$row['accounttitle'] .'" >
