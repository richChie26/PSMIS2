...
$sqlppe = "SELECT a.id, d.accounttitle,item,yearoflife, 
concat(item, ', ',`description`,', ', `Serial`) description, `propertyno`
,(SELECT `Fundcategory` FROM `m_fundcluster` x  WHERE x.isactive =  1 and x.id= c.sourceoffund)  fund 
...
(SELECT x.id , y.`accounttitle`, `item`,`yearoflife`,y.typeofasset FROM `m_equipent` x
 left join m_accouttitle y on x.`accounttitle` = y.id and y.isactive = 1
  where y.typeofasset = 3 and x.isactive =1 ) b on a.`itemid` = b.id 
where a.isactive = 1 and `ResponsibilityCenter` = $Responsibility
and d.typeofasset = 3
"; 
...
while($row = mysqli_fetch_array($qrypee)){
  echo '<tr class= "itemrowppesemi3" id="'.$row['id'].'|'.$row['propertyno'].'|'.$row['item'].'|'.$row['description'].'|'.$row['yearoflife'] .'|'.$row['accounttitle'] .'|'.$row['fund'] .' " >
