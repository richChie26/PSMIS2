...
$PARNO = $_GET['mydata'];
$sqllist = "SELECT f.id ,b.* FROM `t_pardetails` f 
left join (SELECT a.id delitemid, b.accounttitle,item,yearoflife, concat(item, ', ',`description`,', ', `Serial`,', ', `chasisnumber`) description, `propertyno` FROM `t_equipmentdeliverydetails` a
...
where y.typeofasset = 3 and x.isactive =1 ) b on a.`itemid` = b.id where a.isactive = 1 ) b on f.`propertyno` = b.propertyno
where f.parno = '$PARNO'  and f.tag = 0 
