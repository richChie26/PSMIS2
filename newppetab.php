...
$sql = "" ;
  $sqlse = "select id from a_mostaccess where isactive =1 and userid = $uid"; 
    ...
    if(mysqli_num_rows($qry21) > 0 ){
        ...
        left join a_section z on y.section = z.id  and z.isactive =1 
        where x.isactive = 1 ) b on a.`receivedby` = b.userid 
        where a.isactive = 1  and a.tag = 0 and a.createdby = $uid 
        order by a.id Desc
        ";
            }
						...
						while ($row= mysqli_fetch_array($qry)) {
							echo '<tr>
									<td  >'.$row['PARNO'].'</td>
				               		...
				               		<td  >'.$row['Position'].'</td>
                          <td  class="pardet" id="myid|'.$row['PARNO'].'">
                          <center><span class=" glyphicon glyphicon-list-alt"   style="color:#337ab7;"></span></center>
                          </td>
                           <td>
                           <center><a href="#" class="btnprintpar" id="par'. '|'. $row['PARNO'] .'"><span class="glyphicon glyphicon-print" 
                           style="color:#337ab7;"></span></a></center></td> 
               					   </tr>';
