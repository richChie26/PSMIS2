...
FROM `t_equipmentdeliveryheader` WHERE ifnull(substring(`creationdate`,1,4),substring(now(),1,4))  = substring(now(),1,4)";
	...
	$sqlinsertheader = "insert into  `t_equipmentdeliveryheader` (`deliverycode`, `sourceoffund`, `Supplierid`, `PODate`, `PONumber`, `deliverydate`, `deliveryNumber`,`isactive`, `createdby`, `creationdate`,tag)
values('$code', $fundid, $supplierid, '$txtpodate', '$txtPO', '$txtddate', '$txtreceiptn',1, $uid, now(),2)";
 
mysqli_query($con,$sqlinsertheader);

$sqlsearch = "SELECT `id`, `itemid`, `amount`, `description`, `serial`, `createdby`, `tag`, `dateaquire` FROM `t_tempsemipee` WHERE `createdby` = $uid and tag = 2 ";
...
$qryproperty = mysqli_query($con,$sqlproperty);
	...
	$sqlinsertdetails = "insert into  `t_equipmentdeliverydetails`(`ResponsibilityCenter`, `deliverycode`, `itemid`, `propertyno`, `dateaquired`, `description`, `Serial`, `amount`, `isactive`, `createdby`, `creationdate`,tag)
values($Responsibility,'$code', $itemid, '$Property', '$dateaquire', '$description', '$serial', $amount , 1, $uid , now(),2)";
