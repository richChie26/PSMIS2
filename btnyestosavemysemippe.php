...
$uid = $_SESSION["uid"] ;
$myreceiver = $_GET['myreceiver'];
$dtpissued = $_GET['dtpissued'];



$sqlko =" SELECT id,`itemid`, `qty`, `propertyno`  FROM `t_temppar`  where createdby = $uid and tag = 2";
...
while ($row = mysqli_fetch_array($qryko)) {
 ...
 FROM `t_par` where substring(ifnull(`creationdate`,now()),1,4) = substring(now(),1,4)
 and tag = 2 
 ";
 ...
 $parno = $arr['parno'];
 $sqlinsertheader = "insert into `t_par` (`PARNO`, `Dateissue`, `datereceived`, `receivedby`, `issuedby`, `isactive`, `createdby`, `creationdate`,tag)
 values
 ('$parno', '$dtpissued', '$dtpissued', $myreceiver, $uid, 1, $uid, now(),2)";
  ...
  $sqlinsertdetails = "insert into `t_pardetails` (`itemid`, `qty`, `parno`, `propertyno`,tag)
  values($itemid,1,'$parno','$propertyno',2)";
